...
public class MonSchedulerController {
    ...
    public Result<Boolean> save(@Parameter(description = "定时任务对象", required = true)@RequestBody MonScheduler monScheduler) {
        return Result.success("新增成功!",monSchedulerService.saveScheduler(monScheduler));
    }
    ...
    public Result<Boolean> remove(@Parameter(description = "定时任务ID", required = true)@PathVariable Serializable id) {
        return Result.success("删除成功!",monSchedulerService.removeSchedulerById(id));
    }
    ...
    public Result<Boolean> update(@Parameter(description = "定时任务对象", required = true)@RequestBody MonScheduler monScheduler) {
        ...
        return Result.success("更新成功!",monSchedulerService.updateSchedulerById(monScheduler)) ;
    }
    ...
    public List<MonScheduler> list() {
        return monSchedulerService.list();
    }
    ...
    public MonScheduler getInfo(@Parameter(description = "定时任务ID", required = true)@PathVariable Serializable id) {
        return monSchedulerService.getById(id);
    }
    ...
    public Result<RPage<MonScheduler>> page(@Parameter(description = "分页对象", required = true) PageQuery query, @Parameter(description = "查询对象", required = true) MonSchedulerSearchDTO monSchedulerSearchDTO) {
        Page<MonScheduler> monSchedulerPage = monSchedulerService.listMonSchedulerPage(query,monSchedulerSearchDTO);
        return Result.success("请求成功", RPage.build(monSchedulerPage));
    }
